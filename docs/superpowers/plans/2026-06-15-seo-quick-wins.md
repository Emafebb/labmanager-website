...
**Test command:** `npx vitest run` (Vitest, run once). A single file: `npx vitest run src/app/ordini/page.test.tsx`.
...
**Status:** Completed in commit `3e41272` (`fix: remove double brand from orders page title`).
...
- [x] **Step 1: Update the test assertions to the new expected title (they will fail)**
...
- [x] **Step 3: Update `PAGE_TITLE` in the page**
...
**Status:** Completed in commit `dab0ccb` (`feat: include gelateria across SEO copy and metadata`).
...
- [x] **Step 1: Update orders test assertions for the sector list (they will fail)**
...
- [x] **Step 3: Update the orders page (`src/app/ordini/page.tsx`)**
...
- [x] **Step 4: Update homepage hero (`src/components/Hero.tsx`)**
...
- [x] **Step 5: Update Footer, Warehouse, DownloadClient, FAQ**
...
- [x] **Step 6: Update global metadata + structured data (`src/app/layout.tsx`)**
...
- [x] **Step 7: Update `public/llms.txt`**
