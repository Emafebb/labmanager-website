...
export default function NewsletterForm({
  ...
  const inputClassName = isPopup
    ...
    : "w-full px-4 py-3 rounded-lg border border-gray-200 focus:border-gray-400 focus:ring-4 focus:ring-gray-200 outline-none transition-[border-color,box-shadow] duration-200 text-base bg-white placeholder:text-gray-500";
  ...
  const privacyLabelClassName = isPopup
    ? "touch-target inline-flex items-center text-[11px] text-gray-500 leading-relaxed cursor-pointer"
    : "touch-target inline-flex items-center text-sm text-gray-700 leading-relaxed cursor-pointer";
  const buttonClassName = isPopup
    ...
    : "w-full inline-flex items-center justify-center gap-3 bg-primary text-white px-8 py-4 rounded-xl text-base font-semibold transition-[background-color,box-shadow,transform] duration-200 ease-out hover:-translate-y-0.5 hover:bg-primary-dark hover:shadow-[0_10px_15px_-3px_rgba(68,3,175,0.2)] active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-none";
    ...
    return (
      <div role="status" className={`text-center ${successPadding}`}>
        <div className="inline-flex rounded-full bg-green-50 p-4">
          <CheckCircle2 size={32} className="text-green-700" aria-hidden="true" />
        </div>
      ...
      <div className={checkboxWrapClassName}>
        ...
        >
          <label htmlFor={`${id}-privacy`} className={privacyLabelClassName}>
            Acconsento a ricevere la newsletter di LabManager.
          </label>
          <p>
            Ho letto la{" "}
            <a
              ...
              className="touch-target inline-flex items-center font-semibold text-primary underline underline-offset-2 hover:text-primary-dark"
            >
              Privacy Policy
            </a>
